...
def top_20_selling_products_till_2024_view(request):
    ...
    def call_gemini(prompt_text):
        ...
        try:
            ...
            return json.dumps({"predicted_july_sales": 0, "reason": f"Error: {str(e)}"})
...
# from .models import (
#     ProductVariant, OrderLineItem, CustomerPromotion_April,
#     CustomerPromotion_May, CustomerPromotion_June, Prompt, Product
# )
...
#         return render(request, "customer/predictions.html", {
#             "predictions": [],
#             "error": "Prompt with type=MainPrompt not found"
#         })
...
#         # Build prompt - UPDATED to instruct model clearly to respond ONLY with JSON inside <json> tags
#         prompt = f"""{prompt_template}
...
# Q: Based on the above data, predict the total quantity sold in July 2025.

# Return ONLY the JSON enclosed in <json> and </json> tags, with no other text or explanation.

# <json>
# {{
#   "predicted_july_sales": <integer>,
...
#         logger.debug(f"Raw OpenAI response for {sku}: {openai_response}")

#         # Parse response strictly inside <json>...</json> tags
#         try:
#             match = re.search(r"<json>(.*?)</json>", openai_response, re.DOTALL)
#             if match:
#                 json_str = match.group(1).strip()
#             else:
#                 # Fallback: Try to parse entire response directly if tags missing (optional)
#                 json_str = openai_response.strip()
#             parsed = json.loads(json_str)
#             predicted_qty = int(parsed.get("predicted_july_sales", 0))
#             reason = parsed.get("reason", "No reason provided.")
#         except Exception as e:
#             logger.error(f"Failed to parse OpenAI response for SKU {sku}: {e}")
#             predicted_qty = 0
#             reason = "Invalid response from OpenAI."
...
def generate_prompt_view(request):
        ...
        for key in request.POST.keys():
            if key.startswith(f"promo_{i}_"):
                ...
                if len(parts) >= 4:
                    ...
                    promo_summary.setdefault(month, {})[field] = val
    ...
    # 📝 Store in session so handle_prompt can retrieve it later
    request.session["top_items"] = items_data
    ...
    return render(request, "customer/generated_prompt.html", {
        ...
        "top_items": items_data,   # ✅ So hidden inputs are populated
...
def clean_gemini_output(raw_text):
    """Cleans Gemini output to extract JSON even if wrapped in markdown or extra text."""
    text = raw_text.strip()
...
def _extract_items_from_post(request, total_items):
    """Extracts SKU data from POST request into list of dicts."""
    items_data = []
    for i in range(1, total_items + 1):
        sku = request.POST.get(f"sku_{i}")
        product_type = request.POST.get(f"product_type_{i}")
...
def handle_prompt(request):
    if request.method == "POST":
        ...
        if action == "save":
            ...
            if not base_prompt:
                return JsonResponse({"error": "Please save the prompt before predicting."}, status=400)
...
- "reason" (brief explanation with algorithm and adjustments)
...
"""
            ...
            if not api_key:
                ...
                try:
                    ...
                    try:
                        predictions = json.loads(cleaned_output)
